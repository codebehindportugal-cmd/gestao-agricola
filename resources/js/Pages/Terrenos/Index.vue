    ...
    <AuthenticatedLayout>
                    ...
                    >
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                            <div>
                                <div class="flex flex-wrap items-center gap-3">
                                    <h2 class="break-words text-xl font-black sm:text-2xl text-slate-900">{{ terreno.nome }}</h2>
                                    <span
                        ...
                        <div class="mt-6 grid gap-4 sm:grid-cols-2">
                            <div class="rounded-3xl bg-slate-50 p-4">
                                <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Localização</p>
                                <p class="mt-2 text-sm text-slate-700">{{ terreno.localizacao || 'Não definida' }}</p>
                            </div>
                            <div class="rounded-3xl bg-slate-50 p-4">
                                <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Tipo de solo</p>
                                <p class="mt-2 text-sm text-slate-700">{{ terreno.tipo_solo || 'Não definido' }}</p>
                            </div>
                            <div class="rounded-3xl bg-slate-50 p-4">
                                <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Parcelas</p>
                                <p class="mt-2 text-sm text-slate-700">{{ terreno.parcelas_count }}</p>
                            </div>
                            <div class="rounded-3xl bg-slate-50 p-4">
                                <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Centro</p>
                                <p class="mt-2 text-sm text-slate-700">
                                    {{ terreno.latitude || '-' }} / {{ terreno.longitude || '-' }}
                                </p>
                            </div>
                            <div class="rounded-3xl bg-slate-50 p-4 sm:col-span-2">
                                <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Polígono</p>
                                <p class="mt-2 text-sm text-slate-700">
                        ...
                        <div class="mt-5 rounded-3xl bg-emerald-50/50 p-4">
        ...
        <Modal :show="createModalOpen" max-width="2xl" @close="closeCreateModal">
            <div class="p-6 sm:p-8">
                <h2 class="text-2xl font-black text-slate-900">Novo terreno</h2>
                <p class="mt-2 text-sm text-slate-500">
                ...
                <form class="mt-6 grid gap-4 sm:grid-cols-2" @submit.prevent="submitCreate">
                    ...
                    <div>
                        <InputLabel value="Estado" />
                        <select v-model="createForm.estado" class="mt-2 block w-full rounded-2xl border-slate-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                            <option v-for="estado in estadoOptions" :key="estado" :value="estado">
                                {{ estadoLabel(estado) }}
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <TextInput v-model="createForm.localizacao" class="mt-2 block w-full rounded-2xl" />
                        <InputError class="mt-2" :message="createForm.errors.localizacao" />
                    </div>
                    <div>
                        <InputLabel value="Tipo de solo" />
                        <TextInput v-model="createForm.tipo_solo" class="mt-2 block w-full rounded-2xl" />
        ...
        <Modal :show="!!editingTerreno" max-width="2xl" @close="closeEditModal">
            <div class="p-6 sm:p-8">
                <h2 class="text-2xl font-black text-slate-900">Editar terreno</h2>
                <p class="mt-2 text-sm text-slate-500">
                ...
                <form class="mt-6 grid gap-4 sm:grid-cols-2" @submit.prevent="submitEdit">
                    ...
                    <div>
                        <InputLabel value="Estado" />
                        <select v-model="editForm.estado" class="mt-2 block w-full rounded-2xl border-slate-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                            <option v-for="estado in estadoOptions" :key="estado" :value="estado">
                                {{ estadoLabel(estado) }}
                            </option>
                        </select>
                    ...
                    <div>
                        <InputLabel value="Localização" />
                        <TextInput v-model="editForm.localizacao" class="mt-2 block w-full rounded-2xl" />
                        <InputError class="mt-2" :message="editForm.errors.localizacao" />
                    </div>
                    <div>
                        <InputLabel value="Tipo de solo" />
                        <TextInput v-model="editForm.tipo_solo" class="mt-2 block w-full rounded-2xl" />
